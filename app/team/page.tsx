...
const values = [
  { title: "Accuracy Over Speed", desc: "We'd rather be right than first. Every fact is verified against primary sources before publication." },
  { title: "Follow the Money", desc: "Ownership chains, financial flows, and corporate structures reveal who really profits — and who pays the cost." },
  { title: "Transparency", desc: "We show our sources. Every article links to the court documents, corporate filings, and original reporting we built it on." },
  { title: "Public Interest", desc: "We publish because the public has a right to know — not for clicks, outrage, or engagement metrics." },
];

export default function TeamPage() {
  return (
    <div className="hex-bg mx-auto max-w-4xl px-6 py-20">
      <div className="mb-4 tag-cyber inline-block">Editorial Board</div>
      ...
      <div className="mb-20 grid gap-6 md:grid-cols-3">
        {team.map((member) => (
          <div
            key={member.name}
            className="card-futuristic corner-brackets p-6"
          >
            <div className="mb-4 flex h-12 w-12 items-center justify-center border border-border text-cyan">
              {member.icon}
            </div>
            <h3 className="mb-1 font-heading text-lg font-bold text-light">
              {member.name}
            </h3>
            <div className="mb-3 tag-cyber inline-block text-[8px]">
              {member.role}
            </div>
      ...
      <div className="section-label">
        <h2 className="font-mono text-[9px] uppercase tracking-[4px] text-cyan/60">
          <span className="text-cyan/30">// </span>Editorial Values
        </h2>
      </div>

      <div className="grid gap-6 md:grid-cols-2">
        {values.map((v, i) => (
          <div key={i} className="border-l border-cyan/30 pl-5 py-2">
            <h3 className="mb-2 font-heading text-base font-bold text-light">
              {v.title}
            </h3>
